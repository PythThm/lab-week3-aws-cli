...
# COMPLETE THIS PART
instance_id=$(aws ec2 run-instances --image-id $ubuntu_ami --count 1 --instance-type t2.micro --key-name $key_name --security-group-ids $security_group_id --subnet-id $subnet_id --associate-public-ip-address --query 'Instances[0].InstanceId' --region $region --output text)
...
# COMPLETE THIS PART
public_ip=$(aws ec2 describe-instances --instance-ids $instance_id --query 'Reservations[*].Instances[*].PublicIpAddress' --region $region --output text)
